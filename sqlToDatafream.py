...
def getDateRange():
    ...
    rangeDate['yesterday'] = arrow.now().replace(days = -1).format('YYYYMMDD')  # 昨天日的日期
    rangeDate['now'] = arrow.now().format('YYYYMMDDHHmmss')
...
def proessSQL(sql,tdate) :
    #此函数为替换plsql脚本中的变量 为  &start_datetime 为 tdate['startDate']的值
    newsql = sql.replace('&start_datetime',tdate['startDate'])
    newsql = newsql.replace('&end_datetime',tdate['endDate'])
    #newsql = newsql.decode('utf-8')
    return newsql

def executeSQL(sqls,conStr,dateDict):
    '''
    ...
    :param dateDict  可选参数,默认值None 为一个日期的字典:{'endDate': '20180606', 'now': '20180606151840', 'startDate': '20180528', 'today': '20180606', 'yesday': '20180605'}
            dateDict['now'] 为 None 时 不保存excel文件.
    :return: 返回 (文件名,Datafream) 的 列表 文件名为参数中的sql文件名,
             Datafream 为从数据库中取得的表转化为Datafream
    '''
    engine = create_engine(conStr)  #创建数据库引擎
    if dateDict['now']!=None:
        basePath = r'./output/' + dateDict['today']
        getfiles.mkdir(os.path.abspath(basePath))  # 确认文件夹存在,不存在则建立此文件夹
        fileName = os.path.join(os.path.abspath(basePath), dateDict['startDate'] + "_" + dateDict['endDate'] + "_" + dateDict['now'] + ".xlsx")
        excelWriter = pd.ExcelWriter(fileName)  # 创建 excelWriter
        print("Excel文件路径:",fileName)
    else:
        print("未生成Excel文件!")
        excelWriter = None
    ...
    for i,sql in enumerate(sqls) :
        ...
        df = pd.read_sql(proessSQL(sql[1],dateDict), engine)  # read_sql直接返回一个DataFrame对象
...
if __name__=='__main__':
    ...
    sqlDf = executeSQL(sqls,conStr,tdate)                         #连接数据库,执行sql 并返回 Datafream

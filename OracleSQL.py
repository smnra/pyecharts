...
start_datetime = (datetime.date.today() - datetime.timedelta(days=1)).strftime("%Y%m%d") + '12'  # 昨天的日期 '2017102512'
end_datetime = datetime.date.today().strftime("%Y%m%d") + '00' # 今天的日期 '2017102600'



sqlFiles = getfiles.getGzipList(os.getcwd()+ '\\sql\\lte','.SQL')

sqls = []    #存储sql脚本的列表
sheetNames = []    #存放sheet名
for i,sqlFile in enumerate(sqlFiles) :
    sheetNames.append(sqlFile.decode('utf-8').replace(os.getcwd() + '\\lte_sql\\','').split('.')[0])    #将文件名作为sheet名存入
